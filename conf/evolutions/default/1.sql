...
create table token_action (
  ...
  created                   timestamp,
  expires                   timestamp,
  constraint ck_token_action_type check (type in ('PR','EV','TI')),
...
alter table teams add constraint fk_teams_owner_2 foreign key (owner_id) references users (id) on delete restrict on update restrict;
create index ix_teams_owner_2 on teams (owner_id);
alter table teams add constraint fk_teams_playerOne_3 foreign key (player_one_id) references users (id) on delete restrict on update restrict;
create index ix_teams_playerOne_3 on teams (player_one_id);
alter table teams add constraint fk_teams_playerTwo_4 foreign key (player_two_id) references users (id) on delete restrict on update restrict;
