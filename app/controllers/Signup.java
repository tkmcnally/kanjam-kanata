...
import views.html.account.invalidToken;
import views.html.account.passwordForgot;
import views.html.account.passwordReset;
...
public class Signup extends Controller {
    ...
    public static Result forgotPassword(final String email) {
        ...
        return ok(passwordForgot.render(form));
    }
    ...
    public static Result doForgotPassword() {
        ...
        if (filledForm.hasErrors()) {
            ...
            return badRequest(passwordForgot.render(filledForm));
        } else {
    ...
    public static Result resetPassword(final String token) {
        ...
        if (ta == null) {
            return badRequest(invalidToken.render());
        }

        return ok(passwordReset.render(PASSWORD_RESET_FORM.fill(new PasswordReset(token))));
    }
    ...
    public static Result doResetPassword() {
        ...
        if (filledForm.hasErrors()) {
            return badRequest(passwordReset.render(filledForm));
        } else {
            ...
            if (login) {
                ...
                return PlayAuthenticate
                    .loginAndRedirect(ctx(), new MyLoginUsernamePasswordAuthUser(u.email));
            } else {
    ...
    public static Result exists() {
        ...
        flash(Application.FLASH_MESSAGE_KEY,
            Messages.get("kanjam.signup.message.user_exists"));
